...
void prinmt_osabi(unsigned char *e_ident);
...
/**
 * print_magic - this is a function that prints the magic numbers of an ELF header
...
/**
 * print_version - this function prints the versin of an ELF header
 * @e_ident: this is apointer to an array containing the ELF version
 */
...
/**
 * print_entry( - this function prints the entry points of an ELF header
...
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
	printf(" Input Entry point address:      ");
...
/**
 * close_elf - this function closes on ELF file
...
/**
 * main - this function displays the information contained in the ELF header at the beginning of an ELF file
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	if (r == -1)
	{
		free(header);
		close_elf(o);
		dprintf(STDERR_FILENO, "Error: `%s`: No such file exist\n", argv[1]);
		exit(98);
	}
